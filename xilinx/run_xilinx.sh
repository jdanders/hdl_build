...
success_cmd=$(cat <<EOF
       if grep -i -E --color "(^Error:|^Fatal:|UVM_ERROR [^:]|UVM_FATAL [^:])" ${logfile}; then
           echo -e "${RED}# Error detected $C (see ${logfile})"
           false
       else if GREP_COLOR="0;40;1;33" grep -i -P --color \
             "^Warning:" ${logfile}; then
...
fail_cmd=$(cat <<EOF
       GREP_COLOR="0;40;1;33" grep -i --no-group-separator -A1 -E --color "\*?[*#] Warning[ :]" \
           ${logfile}
       grep -i --no-group-separator -A1 -E --color \
